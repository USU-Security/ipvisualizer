...
void datapointdrawpixel(struct datapoint* dp, unsigned char imgdata[][COLORDEPTH], int x, int y, int width)
{
	...
	for (i = x+1; i < x + BLOCKWIDTH; i++) 
	{
		for (j = y+1; j < y + BLOCKHEIGHT; j++) 
		{
			if (i-x > j-y) 
			{
				imgdata[i+j*width][2] = dp->udpin;
				imgdata[i+j*width][1] = dp->tcpin;
				imgdata[i+j*width][0] = dp->otherin;
			} 
			else 
			{
				imgdata[i+j*width][2] = dp->udpout;
				imgdata[i+j*width][1] = dp->tcpout;
				imgdata[i+j*width][0] = dp->otherout;
			}
			if (i-x == j-y || i-x == BLOCKHEIGHT-(j-y)) 
			{
				if (imgdata[i+j*width][2] + dp->drop > 255)
					imgdata[i+j*width][2] = 255;
				else
					imgdata[i+j*width][2] += dp->drop;
				if (imgdata[i+j*width][1] + dp->drop > 255)
					imgdata[i+j*width][1] = 255;
				else
					imgdata[i+j*width][1] += dp->drop;
				if (imgdata[i+j*width][0] + dp->drop > 255)
					imgdata[i+j*width][0] = 255;
				else
					imgdata[i+j*width][0] += dp->drop;
			}
		}
	}
}
